...
if [ -f "/home/${user_name}/.config/ar18/xfce_desktop_deployment/installed_target" ]; then
  target="$(cat "/home/${user_name}/.config/ar18/xfce_desktop_deployment/installed_target")"
  if [ -f "${script_dir}/config/${target}" ]; then
    "${script_dir}/config/${target}"
  fi
fi
